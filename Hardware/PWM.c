...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);		//将JTAG引脚失能，作为普通GPIO引脚使用
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);							//将PB15引脚初始化为复用推挽输出	
																	//受外设控制的引脚，均需要配置为复用模式		
	...
	TIM_OCInitStructure.TIM_OutputNState = TIM_OutputNState_Enable;	//高级定时器特有
	...
	TIM_CtrlPWMOutputs(TIM1, ENABLE);//定时器1开启主输出
